...
def hashcash_check(stamp, resource=None, nbits=None, etime=None, ntime=None):
    ...
    if resource is not None and resource != stamp_split[3]:
        raise AssertionError( "hashcash stamp resource does not match")

    stamp_bits = int(stamp_split[1])
    if nbits is not None and stamp_bits < nbits:
        raise AssertionError("hashcash stamp has less bits than required")
    ...
    if stamp_time > now_time:
        raise AssertionError("hashcash stamp has its date set in the future")
    else:
        if etime is None:
            # By default stamps expire in 28 days.
            etime = 2419200
        if now_time - stamp_time > etime:
            raise AssertionError("hashcash stamp has expired")
